<template>
  <div class="container mt-4">
    ...
    <div v-else>
      ...
      <p class="note mb-4">(Use 000000 as the 6-digit code)</p>
        ...
        <div class="list-group">
          <div v-for="transaction in transactions" :key="transaction.id" class="card mb-3">
            <div class="card-body">
              ...
              <p class="card-text"><span class="label blue">Amount:</span> £{{ (transaction.amount / 100).toFixed(2) }}
              </p>
...
.container {
  ...
  /* Light gray background consistent with other pages */
  border-radius: 8px;
...
@media (min-width: 992px) {
  .container {
    width: 60%;
    /* Use 60% of the screen width on large screens */
  }
}
...
@media (min-width: 768px) and (max-width: 991px) {
  .container {
    width: 70%;
    /* Use 70% of the screen width on medium screens */
  }
}
...
.card:hover {
  ...
  /* Scale up the card by 5% on hover */
}
...
.card-title {
  font-size: 1.25rem;
  /* Slightly larger text for titles within cards */
}

.card-text {
  margin-bottom: 0.5rem;
  /* Consistent spacing for all text inside card */
}
...
.btn-primary {
  ...
  /* Custom green background for buttons */
  border-color: #4CAF50;
...
.btn-primary:hover {
  ...
  /* Darker shade on hover for depth effect */
}

.alert {
  color: #fff;
  /* White text color for alerts */
